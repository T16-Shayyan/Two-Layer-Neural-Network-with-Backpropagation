...
(defun verify-equal (funcname &rest matrices)
  ...
  (when *verify*
    (unless (and
      ...
      (throw-error (format nil "In ~s, matrix dimensions not equal: ~s"
			   funcname
...
(defun verify-multiplicable (matrix1 matrix2)
  ...
  (when *verify*
	...
	(throw-error (format nil "In multiply, matrix dimensions not valid: ~s"
...
(defun sigmoid (u)
  ...
  ;;;apply the sigmoid function to a num
  ;;;uses teh sigmoid funciton given in the siles where B = 1
  (/ 1.0 (+ 1.0 (exp (- u)))))
...
(defun net-error (output correct-output)
  ...
  ;;;finds teh error between the netwrok output and correct output
  ;;;uses formula error =  0.5 ( tr[c - o] . (c - o) )
  ;;;used to measure how well the network is learning
  (* 0.5 (e (multiply (transpose (subtract correct-output output))
...
the datum as input."
  ...
  ;;;performs teh forward pass of the network
  ;;;uses h = sigmoid[v . i] o = sigmoid[w . h]
  ;;;uses map-m to apply sigmoid over the list
  ;;;returns the predicted output vector of the network for this datum
  (let ((h (map-m #'sigmoid (multiply v (first datum)))))
    (let ((o (map-m #'sigmoid (multiply w h))))
      o)))
...
returning a list consisting of new, modified V and W matrices."
  ...
  ;;;uses these formulas odelta = (c - o) o (1 - o) 
  ;;;hdelta = (h (1 - h) (tr[w] . odelta) )
  ;;;w = w + alpha (odelta . tr[h]) v = v + alpha (hdelta . tr[i])
  ;;;first does the forward pass then calculates the output error and hidden error
  ;;;returns the updated v and w matrix
  (let* ((h (map-m #'sigmoid (multiply v (first datum))))
         (o (map-m #'sigmoid (multiply w h)))
         (odelta (e-multiply (subtract (second datum) o)
                             o
                             (subtract-from-scalar 1.0 o)))
         (hdelta (e-multiply h
                             (subtract-from-scalar 1.0 h)
                             (multiply (transpose w) odelta)))
         (new-w (add w (scalar-multiply alpha
                                        (multiply odelta (transpose h)))))
         (new-v (add v (scalar-multiply alpha
                                        (multiply hdelta (transpose (first datum)))))))
    (list new-v new-w)))
...
and the final W matrix of the learned network."
  ...
  ;;;initializes v and w with random wrights
  ;;;loops till max-iterations
  ;;;shuffles training data each iteration
  ;;;calculates mean and worst error every modulo
  ;;;stops early if worst error is less than *a-good-minimum-error*
  ;;;returns the final v and w matrix
  (let* ((v (make-random-matrix num-hidden-units
                                (length (first (first data)))
                                initial-bounds))
         (w (make-random-matrix (length (second (first data)))
                                num-hidden-units
                                initial-bounds)))
    (dotimes (i max-iterations)
      (dolist (element (shuffle data))
        (let ((v-and-w (back-propagate element alpha v w)))
          (setf v (first v-and-w)
                w (second v-and-w))))
      ;;;check the modulo by using mod of i+1
      (if (zerop (mod (+ 1 i) modulo))
          (progn (let* ((error-list (mapcar (lambda (x)
                                              (net-error (forward-propagate x v w)
                                                         (second x)))
                                            data))
                        (worst-error (reduce #'max error-list))
                        (mean-error (average error-list)))
                   (optionally-print error-list print-all-errors)
                   (format t "~%worst error= ~a mean error= ~a" worst-error mean-error)
                   (if (< worst-error *a-good-minimum-error*)
		       ;;;break out the dotimes loop
                       (return))))))
    (list v w)))
...
and use a modulo of MAX-ITERATIONS."
  ...
  ;;;divides data into train and test data and converts both to column vector form
  ;;;trains the neural network using tarin data
  ;;;tests it on the test data finding the average of the net-error of the forward propogate function.
  (let* ((traindata (convert-data
                     (subseq data
                             0
                             (truncate (length data) 2))))
         (testdata (convert-data
                    (subseq data
                            (truncate (length data) 2))))
         (v-and-w (net-build traindata num-hidden-units alpha
                             initial-bounds max-iterations max-iterations nil)))
    (average
     (mapcar (lambda (x)
               (net-error (forward-propagate x (first v-and-w) (second v-and-w))
                          (second x)))
             testdata))))
...
;;;(print (simple-generalization *nand* 3 1.0 5 20000))
...
(print (simple-generalization (scale-data *voting-records*) 13 1.0 5 10000))  ;; pick appropriate values

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(print (simple-generalization (scale-data *mpg*) 8 0.01 0.01 5000)) ;; pick appropriate values

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(print (simple-generalization (scale-data *wine*) 2 0.2 0.5 1000))  ;; pick appropriate values
...
#|
I started the project by reading the whole project, copying the formulas and the given helper functions on a notepad as they would help
in the functions later. After this I started writing the sigmoid function using the given instructions about how the formula can be
transferred into a matrix transpose form. I used 1.0 there for float division. After this I wrote down the net-error function using
the given formula in the documentation. And extracted the scalar form the 1 x 1 matrix using the e matrix function given. Then I 
started finishing the forward-propagate function by using the given formula and the helper function map-m to run a function across
a matrix. After this I started the back-propagate function. Here I learned about about the let* function which was really helpful as
I used 2 let statements in my forward propagate function. Here I also used the given formulas in the comments and the helper functions.
Here I learned about how spaced variables are separated with - instead of an _ (underscore), just a thing I learned about Common Lisps
naming conventions. After this I started implementing the simple-generalization function as net-build looked a bit intimidating. 
In the simple-generalization function I started by realizing that dividing the matrix using could be hard as for odd matrices the 
division would be hard to handle so while looking for other functions I came across truncate which I used to divide the error into 
2 sets and converted them to column vector form and then initialized them to the train and test data. After running the net-build 
function on them I found the average net-error using mapcar (learned this from project 1) and lambda function across the testdata 
in which I found the net-error of the forward-propogate part of the neural network that was trained on the train data. Here I also 
learned how Common Lisp uses nil to represent false. After this I started implementing the net-build function. Here I first went 
through the notes where I found out the Algorithm 11 about Building a Two-Layer Neural Network with Error Backpropagation (Sigmoid), 
the phrase about starting v and w with small random values and ended with reading the professors email about random lists. After all 
that I started implementing the function by initializing v and w using the make-random-matrix function given. Then I started 
implementing the function step by step using the given function header starting by looping using max-iterations 
(using dotimes as a for loop). Then the next point about back-propogating across the shuffle of data. After this I started doing 
the operations that needed to be done each modulo. I used an if statement and learned about how progn is used for multi line if.
Here I made a list of errors and found the mean error using the average function and max error using the reduce (learned in project 1) 
function to loop around the list and find the max error. After this I found the print format string through the lisp resource and 
printed the worst error and mean error using that and the helper optionally-print function. At the end I added the breaking statement
that if the worst error is less than the good-minimum-error then return from the loop. Here I noticed how return-from could be used
(learned how it prematurely exits the loop from the professor) but then realized the instructions said to end the loops so I
used return only. At the end I returned the list of v and w.
|#
